...
import { X, Merge, TrendingUp, Check, ArrowRight } from 'lucide-react';
import { WorkflowProfile } from '../../../types/analysis';
import { Badge } from '../../analysis/Badge';

interface WorkflowMergeModalProps {
  sourceProfile: WorkflowProfile;
  allProfiles: WorkflowProfile[];
  onMerge: (sourceId: string, targetId: string, mergedName: string) => void;
  onClose: () => void;
}

export function WorkflowMergeModal({ sourceProfile, allProfiles, onMerge, onClose }: WorkflowMergeModalProps) {
  ...
  const getMergedPreview = () => {
    ...
    const mergedSteps = [...new Set([
      ...sourceProfile.steps.map(s => s.name.toLowerCase()),
      ...selectedTarget.steps.map(s => s.name.toLowerCase())
    ])];
    const mergedParticipants = [...new Set([...sourceProfile.participants, ...selectedTarget.participants])];
    const mergedSystems = [...new Set([...sourceProfile.systems, ...selectedTarget.systems])];
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">Merge Workflows</h2>
              <p className="text-indigo-100 text-sm">
                Combine "{sourceProfile.name}" with another workflow
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-blue-900">{sourceProfile.name}</h3>
                  <p className="text-sm text-blue-600">
                    {sourceProfile.count} interview{sourceProfile.count !== 1 ? 's' : ''} • {sourceProfile.steps.length} steps • {getFrequencyLabel(sourceProfile.frequency)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Select Workflow to Merge Into
            </label>
            {availableTargets.length === 0 ? (
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-8 text-center">
                <TrendingUp className="w-12 h-12 mx-auto mb-3 text-slate-300" />
                <p className="text-slate-500">No other workflows available to merge with.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {target.count} interview{target.count !== 1 ? 's' : ''} • {target.steps.length} steps
                          </p>
                        </div>
                      </div>
                      {selectedTargetId === target.id && (
          ...
          {selectedTarget && (
            <div>
              ...
              <input
                ...
                placeholder="Enter the name for the merged workflow"
              />
            </div>
          )}
            ...
            <div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-green-900">{mergedName || 'Merged Workflow'}</h3>
                    <p className="text-sm text-green-600">
                      {preview.count} interview{preview.count !== 1 ? 's' : ''} (combined) • {preview.steps.length} steps
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <ul className="text-green-700 space-y-0.5">
                      {preview.steps.slice(0, 4).map((s, i) => (
                        <li key={i} className="truncate capitalize">• {s}</li>
                      ))}
                      {preview.steps.length > 4 && (
                        <li className="text-green-500">+{preview.steps.length - 4} more</li>
                      )}
                    </ul>
                  </div>
                  ...
                  {preview.participants.length > 0 && (
                    <div>
                      <p className="font-medium text-green-800 mb-1">Participants</p>
                      <div className="flex flex-wrap gap-1">
                        {preview.participants.slice(0, 4).map((p, i) => (
                  ...
                  {preview.systems.length > 0 && (
                    <div>
                      <p className="font-medium text-green-800 mb-1">Systems</p>
                      <div className="flex flex-wrap gap-1">
                        {preview.systems.slice(0, 4).map((s, i) => (
                          <Badge key={i} variant="blue" className="text-[10px]">
                            {s}
                          </Badge>
                        ))}
                        {preview.systems.length > 4 && (
                          <Badge variant="gray" className="text-[10px]">
                            +{preview.systems.length - 4}
                          </Badge>
                        )}
